...
server <- function(input, output, session) {
  values <- reactiveValues(apiKey = NULL, results = NULL, image = NULL)
  ...
  observeEvent(input$generateImage, {
    req(values$results)
    example_sentence <- values$results$Details[1]  # Use the first example sentence
    image_url <- dallE(example_sentence, values$apiKey)
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      checkboxInput("generateImage", "Create Picture"),
      uiOutput("image")
    )
  ),
  tags$script(HTML("
