  ...
  observe({
    ...
    output$generateButtons <- renderUI({
      tagList(
        ...
        actionButton("createAnkiCard", "Create Anki Card", style = "margin-top: 20px;")
      )
    })
  })
      ...
      Back = paste0(
        ...
        paste(
          apply(results, 1, function(row) {
            audio_tag <- row['Audio']
            audio_src <- ifelse(!is.na(audio_tag) && nzchar(audio_tag), 
                                sub('.*src="([^"]+)".*', '\\1', audio_tag), "")
            audio_html <- if (nzchar(audio_src)) {
              sprintf('<audio src="%s" type="audio/mp3" controls="controls"></audio>', audio_src)
            } else {
              ""
            }
            sprintf("<tr><td>%s</td></tr><tr><td>%s</td></tr><tr><td>%s</td></tr>", row['Property'], row['Details'], audio_html)
          }),
          collapse = ""
        ),
        "</table>"
